...
namespace SF.UIModule
{
    ...
    /// <remarks>
    ///		There can be multiple PanelControllerBase using the same <see cref="UnityEngine.UIElements.PanelRenderer"/>.
    ///		Example the main menu has sub <see cref="UIView"/> that display a specific menu like inventory, options, and ect.
    ///		Depending on the submenu it might have its own PanelControllerBase class for its logic.
    /// </remarks>
    /// </summary>
    public abstract class PanelControllerBase : UIControllerBase
    {
        // This is here to temporary bridge the gap during
        // the replacement of the UIDocument use with the PanelRenderer.
    }
    
    public abstract class PanelController : PanelControllerBase
    {
        ...
        {
            _panelRenderer.RegisterUIReloadCallback(OnPanelControllerUIReloaded);
        }
		
        protected virtual void OnDisable()
        {
            _panelRenderer.UnregisterUIReloadCallback(OnPanelControllerUIReloaded);
        }
        ...
        protected abstract void OnPanelControllerUIReloaded(PanelRenderer panelRenderer, VisualElement rootElement);
    }
}
